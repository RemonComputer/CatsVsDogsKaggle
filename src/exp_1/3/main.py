...
class Net(nn.Module):
    ...
    def train(self, optimizer, train_dataset, test_dataset, train_batch_size = 32, test_batch_size = 32, no_ephocs = 10, \
                model_save_intervals_in_ephocs = 5, test_accuracy_interval_in_batches = 100, no_test_batches = 5):
...
if __name__ == '__main__':
    ...
    optimizer =  optim.SGD(model_parameters, lr=0.1, momentum=0.9)
    model.train(optimizer, augmented_train_dataset, test_dataset, train_batch_size = 128, test_batch_size = 128, no_ephocs = 10, model_save_intervals_in_ephocs = 2, test_accuracy_interval_in_batches = 10, no_test_batches = 10)
    #model.print_model_parameters()
    original_train_dataset = CatsVsDogsDataset('train', '../../../Dataset/', transform=test_transform)
    model.test(original_train_dataset, 'train', batch_size = 128)
    model.test(test_dataset, 'test', batch_size = 128)
    model.test(validation_dataset, 'validation', batch_size = 128)
    submission_dataset = CatsVsDogsDataset('submission', '../../../Dataset/', transform=test_transform)
    model.generate_submission_file(submission_dataset, 'submission.csv', batch_size = 128)